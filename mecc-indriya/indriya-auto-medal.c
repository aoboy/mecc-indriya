...
static volatile uint8_t synch_rounds_counter = 0;
static volatile uint8_t propg_slot_counter = 0;
static volatile uint8_t propg_slot_flag = 0;
...
/**
 * @brief radio_read_flag: serves as a mutex to prevent a READ/WRITE
...
#define STARTER_ID 1
#else //COOJA
#define STARTER_ID 174 ///change this to other nodeID reflecting the testbed
#endif //COOJA
static uint8_t slot_counter = 0;
/*---------------------------------------------------------------------------*/
PROCESS(node_discovery_process, "medal-disc process");
PROCESS(ndtrigger_process, "node trigger process");
PROCESS(nodeoutput_process, "node-output process");
...
static void clear_vars(){
    ...
    ///aux variables used for propagating synchronization
    propg_slot_counter = 0;
    propg_slot_flag = 0;
    ...
    /*nxt_channel_idx =  random_int(num_channels);
    if (rimeaddr_node_addr.u8[0] == (uint8_t)STARTER_ID){
        cc2420_set_channel(i3e154_channels[0]);
    }else{
...
static char aloha_transmit(void){
    ...
    if (dq != NULL){
        ...
        dp->seqno   = (uint8_t)slot_increment_offset;
        dp->rounds  = synch_rounds_counter;
...
/*---------------------------------------------------------------------------*/
static void start_up_protocol(rtimer_clock_t T_arrival, uint8_t seqno){

        if(rimeaddr_node_addr.u8[0] != (uint8_t)STARTER_ID){
            synch_rounds_counter++;
        }
        ...
        //callback_time = T_arrival + TS/2;
        //compute offset required to start the protocol
        uint8_t time_2_start = (MAX_SYNCH_PKTS  - seqno);

        if(seqno != 0){
            if(time_2_start == 0){
                callback_time = T_arrival + 4;
            }else{
                callback_time = T_arrival + time_2_start*TS;
            }
        }else{
            callback_time = T_arrival + TS/2 + TS/5;
        }
...
static void packet_input(void){
    ...
    int len = 0;
    rtimer_clock_t time_arrival_pkt = RTIMER_NOW();
    ...
    if(pkt->ptype == SYNCH_PACKET){
        ...
        if(nd_is_started == 0 && (rounds_counter < pkt->rounds)){

            //set a flag indicating a new round of the protocol
            nd_is_started = 1;

            ///I'm closer to the starter node..
            propg_slot_flag = 1;
            ...
            start_up_protocol(time_arrival_pkt, pkt->seqno);
        }
    ...
    if(pkt->ptype == DATA_PACKET){
        ...
        if(nd_is_started == 0 && (rounds_counter < pkt->rounds)){
            ...
            ///started by other node else
            propg_slot_flag  = 0;
            propg_slot_counter = pkt->seqno;

            ///turn the radio off
            ///off();
            COOJA_DEBUG_PRINTF("==>%d-*%d-+%d\n", pkt->seqno, nd_is_started, pkt->rounds);

            //trigger another round of measurements
            start_up_protocol(time_arrival_pkt, 0);
        }
        ...
        if(all_neighbors_found_flag == 0){
            ...
            if(all_neighbors_found(num_neighbors)){
                ...
                if(propg_slot_flag == 0){
                    discovery_time = discovery_time +  propg_slot_counter;
                }
                ...
                ///poll the output process for serial output.
...
static void init(void){
  ...
  //initialize with a random channel.
  //nxt_channel_idx =  random_int(num_channels);
  //cc2420_set_channel(i3e154_channels[nxt_channel_idx]);
  cc2420_set_channel(i3e154_channels[0]);
  ...
  process_start(&node_discovery_process, NULL);
   ...
   process_start(&ndtrigger_process, NULL);
...
static char time_syncher(struct rtimer *t, void* ptr){
    ...
    if (slot_counter <=  5){
        ...
        /** we get a free buffer for transmission*/
            ...
            dpkt->rounds= synch_rounds_counter;
            ...
            int r =  rtimer_set(&generic_timer, callback_time, 1,
        ...
        ref_timer = RTIMER_NOW();

        nxt_channel_idx =  random_int(num_channels);
        cc2420_set_channel(i3e154_channels[nxt_channel_idx]);

        NETSTACK_RADIO.on();
        process_post(&nodeoutput_process, PROCESS_EVENT_CONTINUE, "start");
    }
    return 0;
}
/*---------------------------------------------------------------------------*/
/**
 * @brief PROCESS_THREAD:  This process is used to initialize the ND discovery
 *  process on a testbed. A node is selected as an intitiator.
 */
PROCESS_THREAD(ndtrigger_process, ev, data){
    static struct etimer et;

    PROCESS_BEGIN();

    PRINTF("nodeID: %u.%u booted\n",
                  rimeaddr_node_addr.u8[1],
                     rimeaddr_node_addr.u8[0]);
    if (rimeaddr_node_addr.u8[0] == STARTER_ID){
#if COOJA
        ...
        PRINTF("Initializer is nodeID: %u.%u",
                      rimeaddr_node_addr.u8[1],
                         rimeaddr_node_addr.u8[0]);
    }
    ...
    while(1){
         if (rimeaddr_node_addr.u8[0] == (uint8_t)STARTER_ID
                 && (rounds_counter < MAX_REPEAT)){

             //we have comenced ND process
             //nd_is_started = 1;
             ...
             ///SET A HIGHER UPPER BOUND
             upper_bound_time     = upper_bound_time + 20;

             //start the real-time timer.. used to obtain a high resolution timer
             slot_counter = 0;
             callback_time = RTIMER_NOW()+2;
             ref_timer     = callback_time; //auto_channel_hopping
             uint8_t ret = rtimer_set(&generic_timer, callback_time, 1,
                                      (void (*)(struct rtimer *, void *))time_syncher, NULL);
             if(ret){
...
PROCESS_THREAD(nodeoutput_process, ev, data){
    ...
    while(1){
        ...
        //PROCESS_EVENT_CONTINUE PROCESS_EVENT_POLL
        char* command = (char*)data;

        if(command != NULL){
            if(strcmp(command, "output") == 0){
                if(all_neighbors_found_flag){
            ...
            if(strcmp(command, "start") == 0){
                nd_is_started = 1;
                ref_timer = ref_timer + 3*TS;
                start_up_protocol(ref_timer, 6);
            }
        }
